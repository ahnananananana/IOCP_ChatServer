...
#ifdef _DEBUG
#pragma comment(lib, "ChatCommon_debug.lib")
#else
#pragma comment(lib, "ChatCommon.lib")
#endif
	...
	_In_ int       nCmdShow)
{
	...
	CDirectxMgr directxMgr;
	directxMgr.Init(s_hWnd, 480, 520);

	CUIClientMgr uiMgr;
	uiMgr.Init(s_hWnd, directxMgr.GetDevice(), directxMgr.GetContext());

	CChatClient chatClient;
	chatClient.Init();
	chatClient.TryConnect(L"3.39.189.45", 50000);
	...
	while (true)
	{
		...
		{
			directxMgr.Present();
			std::list<tChat> listNewChats = chatClient.GetNewChats();
			uiMgr.AddNewChats(listNewChats);
			uiMgr.Render();
		}
	}
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
	...
	s_hWnd = CreateWindowW(szWindowClass, szTitle, WS_OVERLAPPEDWINDOW,
